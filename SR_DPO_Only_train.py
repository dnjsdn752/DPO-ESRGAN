...
ref_generator.load_state_dict(torch.load("./results/base/g-last.pth"))
...
def train_adversarial(train_dataloader, epoch) -> None:
    ...
    for index, (lr, hr) in enumerate(train_dataloader):
        ...
        for i in range(clamp_sr.size(0)):
            ...
            pie_inverse = -pie #지수함수를 통과하여 크게 만들어주기
        ...
        #dpo loss
        logits = torch.log((max_value[0].detach())/max_value[1]) - torch.log(min_value[0]/min_value[1]) # 선호 - 비선호 형태 (-분모 분자끼리 사라져버림)
        loss_dpo = (
...
def main() -> None:
    ...
    for epoch in range(start_epoch, 205):
