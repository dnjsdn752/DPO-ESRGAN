...
parser.add_argument("--dataset_name", type=str, default="URban100", help="name of the dataset")
...
generator.load_state_dict(torch.load("./results/ref_only_dpo_max-min_nobackward/g-last.pth"))
generator.eval()

Tensor = torch.cuda.FloatTensor if torch.cuda.is_available() else torch.Tensor
image_size = 128

test_dataset = BaseDataset("../data/%s" % opt.dataset_name, image_size, upscale_factor, "original_size_valid")
...
if writing:
    results_file_path = os.path.join(exp_dir2, 'test.txt')
    with open(results_file_path, 'a') as f:  # Append mode
        f.write("mean: [content: %f, psnr: %f, ssim: %f, lpips: %f, pie: %f] \n"
                % (
                    np.mean(content_list),
                    np.mean(psnr_value_list),
                    np.mean(ssim_value_list),
